...
public:
	...
	void Transpose()
	{
		...
		float4x4 This = *this;						   // 0   , 0, -1		 // 0 , 100,  1
		Identity();									   // 100 , 1,  0	->	 // 0,    1,  0
		for (size_t y = 0; y < YCount; y++)			   // 1   , 0,  0		 // -1 ,  0,  0
		{
	...
	{
		Identity();							   //100, 0 , 0 , 0
		Arr2D[0][0] = _Value.x;				   // 0 ,100, 0 , 0
		Arr2D[1][1] = _Value.y;				   // 0 , 0 ,100, 0
		Arr2D[2][2] = _Value.z;				   // 0 , 0 , 0 , 1
	}
	...
	{
		Identity();						  //  0   1   2   3
		Arr2D[3][0] = _Value.x;			  //0 0,  0 , 0 , 0
		Arr2D[3][1] = _Value.y;			  //1 0 , 0,  0 , 0
		Arr2D[3][2] = _Value.z;			  //2 0 , 0 , 0 , 0
	}									  //3 200, 200 , 200 , 1
